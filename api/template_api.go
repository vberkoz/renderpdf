...
const (
	templateResource           = "/api/v1/templates"
	dashboardTemplatesResource = "/api/v1/dashboard/templates"
)
...
type starterTemplateReply struct {
	...
	ExampleVariables map[string]any            `json:"exampleVariables"`
}
...
func isTemplateRequest(request events.APIGatewayProxyRequest) bool {
	...
	return isTemplateResourcePath(path)
}

func isTemplateResourcePath(path string) bool {
	return path == templateResource || strings.HasPrefix(path, templateResource+"/") ||
		path == dashboardTemplatesResource || strings.HasPrefix(path, dashboardTemplatesResource+"/")
}
...
func handleTemplateRequest(ctx context.Context, request events.APIGatewayProxyRequest, headers map[string]string, store templateStore) events.APIGatewayProxyResponse {
	...
	case http.MethodPost:
		...
		now := time.Now().UTC()
		template := Template{ID: uuid.NewString(), Name: input.Name, Type: input.Type, HTML: input.HTML, Variables: input.Variables, OwnerID: ownerID, CreatedAt: now, UpdatedAt: now, Version: 1}
	...
	case http.MethodPut:
		...
		existing.Name, existing.Type, existing.HTML, existing.Variables = input.Name, input.Type, input.HTML, input.Variables
...
func starterTemplateReplies() []starterTemplateReply {
	...
	for _, starter := range starters {
		replies = append(replies, starterTemplateReply{Name: starter.Name, Type: starter.Type, HTML: starter.HTML, Variables: starter.Variables, ExampleVariables: starter.ExampleVariables})
	}
	return replies
}
...
func templateRequestSegments(request events.APIGatewayProxyRequest) []string {
	...
	if path == dashboardTemplatesResource || strings.HasPrefix(path, dashboardTemplatesResource+"/") {
		resource = dashboardTemplatesResource
	}
	remainder := strings.Trim(strings.TrimPrefix(path, resource), "/")
	if remainder == "" {
		return nil
	}
