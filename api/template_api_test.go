...
func dashboardTemplateAPIRequest(method, ownerID, path, body string) events.APIGatewayProxyRequest {
	request := events.APIGatewayProxyRequest{HTTPMethod: method, Path: path, Body: body}
	if ownerID != "" {
		request.RequestContext.Authorizer = map[string]interface{}{
			"claims": map[string]interface{}{"sub": ownerID},
		}
	}
	return request
}
...
func TestTemplateCRUDUsesOwnerScopeAndExpectedStatuses(t *testing.T) {
	store := &memoryTemplateAPIStore{}
	create := handleTemplateRequest(context.Background(), templateAPIRequest("POST", "customer-a", templateResource, `{"name":"Invoice","type":"invoice","html":"<p>{{customer.name}}</p>","variables":{"customer":{"name":"Ada Lovelace"}}}`), nil, store)
	...
	if created.Variables["customer"].(map[string]interface{})["name"] != "Ada Lovelace" {
	...
	update := handleTemplateRequest(context.Background(), templateAPIRequest("PUT", "customer-a", templateResource+"/"+created.ID, `{"name":"Updated invoice","type":"invoice","html":"<p>{{customer.name}}</p>","variables":{"customer":{"name":"Grace Hopper"}}}`), nil, store)
	if update.StatusCode != 200 {
		t.Fatalf("update status = %d, want 200: %s", update.StatusCode, update.Body)
	}
	var updated Template
	if err := json.Unmarshal([]byte(update.Body), &updated); err != nil {
		t.Fatalf("decode update response: %v", err)
	}
	if updated.Variables["customer"].(map[string]interface{})["name"] != "Grace Hopper" {
...
func TestDashboardTemplateRoutesUseCognitoSubject(t *testing.T) {
	store := &memoryTemplateAPIStore{}
	created := handleTemplateRequest(context.Background(), dashboardTemplateAPIRequest("POST", "customer-a", dashboardTemplatesResource, `{"name":"Invoice","type":"invoice","html":"<p>{{customer.name}}</p>"}`), nil, store)
	if created.StatusCode != 201 {
		t.Fatalf("dashboard create status = %d, want 201: %s", created.StatusCode, created.Body)
	}
	var template Template
	if err := json.Unmarshal([]byte(created.Body), &template); err != nil {
		t.Fatalf("decode dashboard create: %v", err)
	}
	if template.OwnerID != "customer-a" {
		t.Fatalf("owner = %q, want customer-a", template.OwnerID)
	}
	response := handleTemplateRequest(context.Background(), dashboardTemplateAPIRequest("GET", "customer-a", dashboardTemplatesResource+"/"+template.ID, ""), nil, store)
	if response.StatusCode != 200 {
		t.Fatalf("dashboard get status = %d, want 200", response.StatusCode)
	}
}
