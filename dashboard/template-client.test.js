...
(async () => {
    ...
    assert.deepStrictEqual(calls.map(({ url, options }) => [url, options.method]), [
        ['/api/v1/templates', 'GET'], ['/api/v1/templates', 'POST'], ['/api/v1/templates/template%2Fa', 'PUT'], ['/api/v1/templates/template%2Fa', 'DELETE'], ['/api/v1/render-template', 'POST']
    ]);
    assert.strictEqual(calls[1].options.headers.Authorization, 'Bearer key');
    assert.deepStrictEqual(JSON.parse(calls[4].options.body), { templateId: 'template-a', variables: { customer: { name: 'Ada' } } });
